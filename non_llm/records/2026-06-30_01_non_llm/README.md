...
1. **`torch.compile` the policy** (new `--compile` flag). The training step is GPU-bound — ~86% of each iter is the conv encoder + minGRU core + Muon, all small ops on a 2.25M-param net. Inductor fusion cuts the per-iter cost **~30%** (1.04 → ~0.72 s/iter on one H100). `cudnn.benchmark=True` is also enabled (autotunes the conv kernels, lossless).
...
**Timing — modded-nanogpt convention.** The one-time `torch.compile` cost is paid in an **untimed warmup**; the model + optimizer are then **reset to their initial state** and the timer starts, so the record times the *full* schedule from step 1 at compiled speed — exactly like the (uncompiled) records #1/#2.
