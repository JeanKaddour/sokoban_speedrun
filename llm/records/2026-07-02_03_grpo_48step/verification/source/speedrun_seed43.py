...
_STRATEGY_MODULE: Any | None = None
_STRATEGY_MODULE_LOADED = False


def _load_strategy_module() -> Any | None:
    """Load an optional local strategy module for record experiments."""
    global _STRATEGY_MODULE, _STRATEGY_MODULE_LOADED
    if _STRATEGY_MODULE_LOADED:
        return _STRATEGY_MODULE
    _STRATEGY_MODULE_LOADED = True
    module_name = os.environ.get("SOKOBAN_STRATEGY_MODULE", "").strip()
    if not module_name:
        return None
    _STRATEGY_MODULE = importlib.import_module(module_name)
    return _STRATEGY_MODULE


def _coerce_strategy_advantages(value: Any, reference: torch.Tensor) -> torch.Tensor:
    if value is None:
        return reference
    if isinstance(value, torch.Tensor):
        out = value.to(device=reference.device, dtype=torch.float32)
    else:
        out = torch.as_tensor(value, dtype=torch.float32, device=reference.device)
    if out.shape != reference.shape:
        raise ValueError(f"strategy advantage shape {tuple(out.shape)} != {tuple(reference.shape)}")
    if not torch.isfinite(out).all():
        raise ValueError("strategy returned non-finite advantages")
    return out


def apply_strategy_advantages(
    ...
    *,
    ...
    fn = getattr(strategy, "transform_group_advantages", None) if strategy is not None else None
    if not callable(fn):
        return advantages
    updated = fn(
    ...
    return _coerce_strategy_advantages(updated, advantages)


def apply_strategy_group_filter(
    rewards: torch.Tensor,
    advantages: torch.Tensor,
    *,
    default_train: bool,
    ...
    strategy = _load_strategy_module()
    fn = getattr(strategy, "should_train_group", None) if strategy is not None else None
    if not callable(fn):
    ...
    return default_train if decision is None else bool(decision)
...
class RunLogger:
    ...
    def __init__(self, run_dir: Path, args: argparse.Namespace, enabled: bool) -> None:
        ...
        try:
            ...
            source_sha256 = self._save_source_snapshot(run_dir, source)
    ...
    @staticmethod
    def _save_source_snapshot(run_dir: Path, source: str) -> str | None:
        if source.startswith("<source unavailable:"):
            return None
        try:
            source_dir = run_dir / "source"
            source_dir.mkdir(parents=True, exist_ok=True)
            source_path = source_dir / "speedrun.py"
            ...
            print0(f"RunLogger warning: could not write source snapshot: {exc!r}")
            return None

    @staticmethod
    def _header(args: argparse.Namespace, source: str, source_sha256: str | None) -> str:
        env_lines = []
        ...
        env_lines.append(f"SOKOBAN_STRATEGY_MODULE: {os.environ.get('SOKOBAN_STRATEGY_MODULE', '')}")
        if source_sha256:
            env_lines.append(f"source snapshot: source/speedrun.py sha256:{source_sha256}")
        else:
            env_lines.append("source snapshot: unavailable")
        try:
            import subprocess
...
def run_pipeline(
    ...
    def _collect_step_batch(step: int, t_step_start: float, times: dict[str, float]) -> CollectStats:
        ...
        while cs.puzzles_used < args.examples_per_step:
            ...
            group_index = cs.groups_seen_total
            adv = compute_group_advantages(rewards_t, args.advantage_mode)
            adv = apply_strategy_advantages(
                rewards_t,
                adv,
                mode=args.advantage_mode,
                step=step,
                num_steps=num_steps,
                group_index=group_index,
                puzzle_id=pid,
            ...
            if os.environ.get("SOKOBAN_STRATEGY_MODULE"):
                default_train_group = group_has_signal(adv)
            else:
                default_train_group = group_has_signal(rewards)
            train_group = apply_strategy_group_filter(
                rewards_t,
                adv,
                default_train=default_train_group,
                step=step,
                num_steps=num_steps,
                group_index=group_index,
                puzzle_id=pid,
            )
            is_zero_var = args.zero_variance_filter and not train_group
