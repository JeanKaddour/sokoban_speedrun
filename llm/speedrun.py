...
def policy_gradient_loss_from_token_logprobs(
    ...
    CISPO and DPPO are zero at IGNORE positions by construction; grpo/gspo objectives
    are exp(log_ratio)-shaped and must be explicitly masked."""
    ...
    if behavior_logprobs is not None:
        ...
        if loss_fn == "cispo":
            ...
            if dppo_delta is None or dppo_delta <= 0:
                raise ValueError("dppo_delta must be a positive float for the dppo loss")
            if dppo_is_cap is None or dppo_is_cap <= 0:
                raise ValueError("dppo_is_cap must be a positive float for the dppo loss")
            ...
            if dppo_divergence == "binary_tv":
                ...
                divergence = (
                    policy_prob * (policy_prob.log() - behavior_prob.log())
                    + (1.0 - policy_prob)
                    * ((1.0 - policy_prob).log() - (1.0 - behavior_prob).log())
                ).clamp(min=0.0)
            else:
                raise ValueError(f"unknown dppo_divergence: {dppo_divergence!r}")
...
def chunked_policy_loss(model, input_ids, attention_mask, labels, advantages, *,
    ...
    """Memory-efficient policy loss (cispo/dppo/grpo/gspo): run the base model to hidden states
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--loss-fn", choices=["cispo", "dppo", "grpo", "gspo"], default="cispo",
                        help="Policy loss. 'cispo' (default, record 1): stop-grad clipped IS weight times "
                             ...
                             "'dppo': mask tokens outside a divergence trust region. "
    ...
    parser.add_argument("--dppo-divergence", choices=["binary_tv", "binary_kl"],
                        default="binary_tv", help="DPPO token divergence approximation")
    parser.add_argument("--dppo-delta", type=float, default=None,
                        help="DPPO trust-region threshold (default: 0.2 TV, 0.05 KL)")
    parser.add_argument("--dppo-is-cap", type=float, default=5.0,
                        help="DPPO importance-weight cap")
...
def _resolve_loss_args(args: argparse.Namespace) -> None:
    ...
    if args.loss_fn == "dppo":
        ...
        if args.dppo_is_cap is None or args.dppo_is_cap <= 0:
