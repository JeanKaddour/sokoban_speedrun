...
#   MAX_STEPS=100 RUN_NAME=<RUN> modal run --detach modal_app.py
#   EVAL_CHECKPOINT=/vol/outputs/<RUN>/step_000099 RUN_NAME=<RUN> modal run modal_app.py
#
...
STEP="${STEP:-000099}"           # final checkpoint step for a 100-step run
...
modal volume ls "$VOL" "/outputs/$RUN"
log_name="$(modal volume ls "$VOL" "/outputs/$RUN" | grep -oE 'log_[0-9a-f]{8}\.txt' | head -1)"
...
uv run --with matplotlib --with pandas --with pillow python plot_hero_animation.py "$RECORD" --out records/hero.gif
echo
if [ -n "$VERIFY_OF" ]; then
